        ...
        function update(elapsedTime) {
          if (!lost) {
            ...
            if (racer_timer > racer_rate) {
              racer_timer = 0;
              var racer = new Racer(
                { x: 128, y: -64, direction: 1 },
                speedMultiplier
              );
              var racer2 = new Racer(
                { x: 320, y: canvas.height + 64, direction: -1 },
                speedMultiplier
              );
              racers.push(racer);
              racers.push(racer2);
            ...
            if (mini_timer > mini_rate) {
              mini_timer = 0;
              var mini = new Mini(
                { x: 192, y: -64, direction: 1 },
                speedMultiplier
              );
              var mini2 = new Mini(
                { x: 448, y: canvas.height + 64, direction: -1 },
                speedMultiplier
              );
              minis.push(mini);
              minis.push(mini2);
            ...
            if (fullLog_timer > fullLog_rate) {
              fullLog_timer = 0;
              var fullLog = new FullLog(
                { x: 704, y: canvas.height, direction: -1 },
                speedMultiplier
              );
            ...
            if (shortLog_timer > shortLog_rate) {
              shortLog_timer = 0;
              var shortLog = new ShortLog(
                { x: 640, y: -128, direction: 1 },
                speedMultiplier
              );
            ...
            if (turtle_timer > turtle_rate) {
              ...
              var turtle = new Turtle(
                { x: 576, y: canvas.height, direction: -1 },
                speedMultiplier
              );
              turtles.push(turtle);
            ...
            if (player.x > 512) {
              var adjustedY =
                player.height *
                Math.floor((player.y + player.height / 2) / player.height);
    ...
    2: [
      function(require, module, exports) {
        ...
        function checkForCollision(entity1, entity2, callback) {
          var collides = !(
            entity1.x + entity1.width - 10 < entity2.x ||
    ...
    3: [
      function(require, module, exports) {
        ...
        function Game(screen, instructions, updateFunction, renderFunction) {
          ...
          this.instructionsCtx.fillText('Frogger Clone Instructions', 250, 50);
          this.instructionsCtx.font = '20px Arial';
          this.instructionsCtx.fillText(
            'Use the Arrow Keys or WASD to move.',
            250,
            125
          );
          this.instructionsCtx.fillText(
            'Try to reach the end, but be careful. Not only are the vehicles deadly,',
            120,
            160
          );
          this.instructionsCtx.fillText(
            'but the threat of drowning also terrifies this frog.',
            215,
            195
          );
    ...
    4: [
      function(require, module, exports) {
        ...
        FullLog.prototype.update = function(time) {
          this.y +=
            (this.direction * this.speed * this.speedMultiplier * time) / 100;
      ...
      function(require, module, exports) {
        ...
        Mini.prototype.update = function(time) {
          this.y +=
            (this.direction * this.speed * this.speedMultiplier * time) / 100;
          if (
            (this.direction == -1 && this.y < -64) ||
            (this.direction == 1 && this.y > 704)
          )
            this.destroy = true;
        };
          ...
          switch (this.state) {
            ...
            case 'moving':
              ...
              if (this.moveTimer <= 1030 / this.speed) {
                ...
                if (!this.isFloating) {
                  if (this.currentDirection.up && !(this.y == 0))
                    this.y -= (this.speed * this.height * time) / 1000;
                  else if (this.currentDirection.down && !(this.y == 640))
                    this.y += (this.speed * this.height * time) / 1000;
                  else if (this.currentDirection.right && !(this.x == 768))
                    this.x += (this.speed * this.width * time) / 1000;
                  else if (this.currentDirection.left && !(this.x == 0))
                    this.x -= (this.speed * this.width * time) / 1000;
                } else {
                  this.isFloating = false;
                  if (this.currentDirection.up && !(this.y == 0))
                    this.y -=
                      (this.speed * this.height * time) / 1000 -
                      this.floatSpeed;
                  else if (this.currentDirection.down && !(this.y == 640))
                    this.y +=
                      (this.speed * this.height * time) / 1000 +
                ...
                this.moveTimer <
                1000 / this.speed + this.moveDelayTime
              )
                this.frame = 0;
              else {
                this.stillMoving = false;
          ...
          switch (this.state) {
            case 'idle':
              ctx.drawImage(
                ...
                this.spritesheet,
                // source rectangle
                this.directionFrame * 256 + this.blinkFrame * 64,
                64,
                this.width,
                this.height,
                // destination rectangle
            ...
            case 'moving':
              ctx.drawImage(
                ...
                this.spritesheet,
                // source rectangle
                this.directionFrame * 256 + this.frame * 64,
                0,
                this.width,
                this.height,
                // destination rectangle
    ...
    10: [
      function(require, module, exports) {
        ...
        function Turtle(position, speedMultiplier) {
          ...
          this.destroy = false;
          //Delay between dives
          this.diveRate = Math.floor(Math.random() * 4 + 4) * 1000;
          this.underTime = 2000;
          this.state = 'up';
        }
